...
export default function ChapterDetailPage() {
      ...
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <main className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-2xl font-bold mb-2">Phần không tìm thấy</h1>
            <button
              onClick={() => router.push("/chapters")}
              className="text-primary hover:text-primary-dark transition-colors"
            >
              Quay lại danh sách nội dung học
            </button>
          </div>
        </main>
        <Footer />
  ...
  const previousChapter =
    currentIndex > 0 ? chaptersData[currentIndex - 1] : null;
  const nextChapter =
    currentIndex < chaptersData.length - 1
      ? chaptersData[currentIndex + 1]
      : null;
      ...
      <main className="flex-1">
        ...
        <section className="py-12 px-4">
          <div className="max-w-4xl mx-auto">
            {/* Sections */}
            <div className="mb-12">
              <h2 className="text-2xl font-bold mb-6">Nội dung học</h2>
